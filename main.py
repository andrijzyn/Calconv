...
def process_input(user_input: str) -> str:
    def parse_math_expression(expression):
        ...
        pattern = r'^([\da-fA-F]+)_(\d+)\s*([+\-*/])\s*([\da-fA-F]+)_(\d+)$'
...
def main(args=None):
    ...
    if parsed_args.expression:
        try: print(process_input(parsed_args.expression))
        except ValueError as ve: print(f"Error: {ve}")
        return

    while True:
        try: print(process_input(input("Enter expression like (x_base1 + y_base2) or (number_base1toBase2): ")))
        except ValueError as ve: print(f"Error: {ve}")
